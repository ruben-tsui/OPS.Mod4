...
class MplCanvas(FigureCanvas):
    ...
    def __init__(self):
        ...
        gs = plt.GridSpec(2, 4, wspace=0.2, hspace=0.3)
        #plt.axis('tight')
        ax1 = self.fig.add_subplot(gs[:1, :3])
        ax2 = self.fig.add_subplot(gs[1:, :3], sharex=ax1)
        ax3 = self.fig.add_subplot(gs[1:, 3:]) # histogram of return distribution
        ax4 = self.fig.add_subplot(gs[:1, 3:]) # histogram of stochastic volatility distr. (Heston model only)
